...
def unblockify(blocks, h, w, block_size=8):
    # Ensure the total size matches the target dimensions
...
def motion_estimate(ref, target, block_size=8, search_range=32):
    ...
    for i in range(0, h, block_size):
        for j in range(0, w, block_size):
            ...
            if block.shape != (block_size, block_size):  # Skip invalid blocks
                continue
            best_match = (0, 0)
            min_error = float('inf')
            ...
            while step >= 1:
                for dy in range(-step, step + 1, step):
                    for dx in range(-step, step + 1, step):
                        ...
                        if ref_block.shape != (block_size, block_size):  # Skip invalid blocks
                            continue
            ...
            y, x = i + dy, j + dx
            ref_block = ref[y:y + block_size, x:x + block_size]
            if ref_block.shape == block.shape:  # Ensure shapes match
...
def motion_estimate_block(args):
    ...
    for dy in range(-search_range, search_range + 1):
        for dx in range(-search_range, search_range + 1):
            y, x = i + dy, j + dx
            if y < 0 or x < 0 or y + block_size > ref.shape[0] or x + block_size > ref.shape[1]:
                continue
            ref_block = ref[y:y + block_size, x:x + block_size]
            error = np.sum(np.abs(ref_block - block))
            if error < min_error:
...
def motion_compensate(ref, mv, residuals, block_size=8):
    ...
    for i in range(0, h, block_size):
        for j in range(0, w, block_size):
            ...
            if ref_block.shape == residual.shape:  # Ensure shapes match
...
class VideoProcessor:
    ...
    def load(self, path, scale=0.5):
        ...
        while True:
            ...
            h, w, _ = frame.shape
            if h % 8 != 0 or w % 8 != 0:
                # Resize to ensure dimensions are divisible by 8
                h = (h // 8) * 8
                w = (w // 8) * 8
    ...
    def compress_inter(self, gop, q, search_range=8):  # Reduce default search range
        compressed = []
        for i in range(0, len(self.frames_yuv), gop):
            ...
            # P-frames (always reference the I-frame)
            ref = y.copy()
            for j in range(i + 1, min(i + gop, len(self.frames_yuv))):
                y_p = self.frames_yuv[j][:, :, 0]
                mv, res = motion_estimate(ref, y_p, search_range=search_range)  # Use reduced search range
        ...
        serialized = pickle.dumps({"type": "inter", "data": compressed, "gop": gop, "q": q})
        self.compressed_data = zlib.compress(serialized, level=9)
    ...
    def save_video(self, path):
        ...
        # Try MP4V first, fall back to XVID if not available
        fourcc = cv2.VideoWriter_fourcc(*"MP4V") if cv2.VideoWriter_fourcc(*"MP4V") != -1 else cv2.VideoWriter_fourcc(*"XVID")
