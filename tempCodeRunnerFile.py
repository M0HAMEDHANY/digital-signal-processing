...
class MainWindow(QMainWindow):
    ...
    def save_video(self):
        ...
        try:
            ...
            # كتابة الإطارات مع شريط تقدم
            progress = QProgressDialog("Saving video...", "Cancel", 0, len(self.vp.decoded), self)
            progress.setWindowTitle("Saving Progress")
            progress.setWindowModality(Qt.WindowModal)
            progress.setMinimumDuration(0)
            
            for i, frame in enumerate(self.vp.decoded):
                out.write(frame)
                progress.setValue(i)
            ...
            if progress.wasCanceled():
                os.remove(path)  # حذف الملف غير المكتمل
                QMessageBox.warning(self, "Cancelled", "Video saving was cancelled.")
            else:
                QMessageBox.information(
                    self,
                    "Success",
